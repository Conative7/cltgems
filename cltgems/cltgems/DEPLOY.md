...
## 4. After pull / upload (Vercel or GitHub)

If this repo is connected to **Vercel**, a push or upload to GitHub triggers an auto-deploy — no extra step beyond verifying the production URL.

If you are updating GitHub without the full monorepo context: re-upload the nested **`cltgems`** folder (the Next.js app root with `package.json`) so the remote matches what you built locally. Exclude `node_modules`, `.next`, and `.git` from zips.
